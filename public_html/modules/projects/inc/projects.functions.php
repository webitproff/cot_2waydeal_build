								  ...
								  $pagepath_home = false, $emptytitle = '')
{
	...
	if ($item_data['item_id'] > 0 && !empty($item_data['item_title']))
	{
		...
		$text_cut = ((int)$textlength > 0) ? cot_string_truncate(strip_tags($text), $textlength) : $text;
//cot_string_truncate(strip_tags($text), 120, false)
		...
		$temp_array = array(
			...
			'USER_PRDURL' => cot_url(
                'users',
					['m' => 'details', 'id' => $item_data['item_userid'] ?? null,  'u' => $item_data['user_name'] ?? null, 'tab' => 'projects']),
			'TITLE' => $itempath,
			...
			'TYPE' => $projects_types[$item_data['item_type']] ?? null,
