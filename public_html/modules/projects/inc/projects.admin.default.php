...
if (!empty($prj_arr) && in_array($mass_act, array('delete', 'validate'))) {
		switch ($mass_act) {
			case 'delete':
...
// Проверяем, является ли $projects_types массивом
if (is_array($projects_types)) {
    foreach ($projects_types as $i => $pr_type) {
        $t->assign([
            'TYPE_ROW_TITLE' => htmlspecialchars((string) $pr_type, ENT_QUOTES, 'UTF-8'),
...
$t->assign([
    ...
    'SEARCH_SQ' => cot_inputbox('text', 'sq', htmlspecialchars((string) $sq, ENT_QUOTES, 'UTF-8'), ['class' => 'schstring']),
    'SEARCH_STATE' => cot_radiobox(
        (int) $state,
        'state',
        [0, 1, 2],
        ['опубликованные', 'скрытые', 'на проверке']
    ),
    ...
    'SEARCH_SORTER' => cot_selectbox(
        ...
        'sort',
        ['', 'costasc', 'costdesc'],
        [$L['projects_mostrelevant'], $L['projects_costasc'], $L['projects_costdesc']],
        false
    ),
    ...
    'CATALOG' => cot_build_structure_projects_tree('', [$c]),
    'CATTITLE' => !empty($c) && !empty($structure['projects'][$c]['title'])
        ? ' / ' . htmlspecialchars((string) $structure['projects'][$c]['title'], ENT_QUOTES, 'UTF-8')
        : '',
]);
