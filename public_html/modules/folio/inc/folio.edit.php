...
$r = cot_import('r', 'G', 'TXT'); // Импортируем параметр r
...
$item['item_parser'] = cot_import('rparser', 'P', 'ALP');
// Устанавливаем парсер по умолчанию, если он пустой
if (empty($item['item_parser'])) {
    $item['item_parser'] = $cfg['folio']['parser'] ?: 'html';
}
...
$t->assign(array(
	...
	'PRDEDIT_FORM_DATE' => cot_selectbox_date($item['item_date'], 'long', 'rdate', cot_date('Y', $sys['now']), cot_date('Y', $sys['now']) - 5),
	"PRDEDIT_FORM_PARSER" => cot_selectbox($item['item_parser'], 'rparser', cot_get_parsers(), cot_get_parsers(), false),
	"PRDEDIT_FORM_DELETE" => cot_radiobox(0, 'rdelete', [1, 0], [$L['Yes'], $L['No']]),
)); 
